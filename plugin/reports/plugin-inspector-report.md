...
| -------- | -------- | ------------ | ------------------------------------------------ | ----------------- | ------------------ |
| openclaw | high     | dynamic-tool | after_tool_call, before_tool_call, gateway_start | definePluginEntry | tools              |
...
| -------- | ---------------------- | ----- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------- |
| openclaw | seam-inventory         | log   | observed 3 hooks, 1 registrations, and 1 manifest contracts                           | hook:after_tool_call, hook:before_tool_call, hook:gateway_start, registration:definePluginEntry, manifestContract:tools | -             |
| openclaw | hook-names-present     | log   | all observed hooks exist in the target OpenClaw hook registry                         | after_tool_call, before_tool_call, gateway_start                                                                        | -             |
| openclaw | api-registrars-present | log   | all observed api.register* calls exist in the target OpenClaw plugin API builder      | -                                                                                                                       | -             |
| openclaw | sdk-exports-present    | log   | all observed plugin SDK imports exist in target OpenClaw package exports              | openclaw/plugin-sdk/plugin-entry                                                                                        | -             |
| openclaw | package-metadata       | log   | selected package metadata for plugin contract checks                                  | package.json, @ctrlrun/openclaw, version:0.1.1                                                                          | -             |
| openclaw | declarative-contracts  | log   | fixture declares manifest contracts that can be checked without executing plugin code | tools                                                                                                                   | -             |
